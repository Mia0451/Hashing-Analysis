...
public class MultiplicationMethodHashTable {
    ...
    private int hashCustomer(Customer customer) {
        int key = customer.hashCode();
        float A = 0.94652f;
        float tmp = abs(key * A);
        int beforeDot = (int) tmp;
        float afterDot = tmp - beforeDot;
        return (int)(afterDot * M);
    }
}
